...
biased_data <- subset(data, data$BiasScore > 0.8 | data$BiasScore < -0.8)
neutral_data <- subset(data, data$BiasScore <= 0.8 & data$BiasScore >= -0.8)

# add group labels
biased_data$AttentionGroup = "biased"
neutral_data$AttentionGroup = "neutral"

# bind back together
data_grouped <- rbind(biased_data, neutral_data)

data_grouped %>%
  ggplot(aes(x = BiasScore, fill = AttentionGroup))+ geom_vline(mapping = aes(xintercept = biasMean),
...
```{r}
data_grouped %>%
  ggplot(aes(x = Incon.ConRT, fill = AttentionGroup, color = AttentionGroup))+
  geom_density(alpha = 0.3)+
  ggtitle("Distribution of Incongruency RT Effects By Bias Group Split")

data_grouped %>%
  ggplot(aes(x = InCon_RT, fill = AttentionGroup, color = AttentionGroup))+
  geom_density(alpha = 0.3)+
  ggtitle("Distribution of Incongruent RT By Bias Group Split")

# means for v-lines
neutral_incon_mean = mean(neutral_data$InCon_RT)
neutral_con_mean = mean(neutral_data$Con_RT)
biased_incon_mean = mean(biased_data$InCon_RT)
biased_con_mean = mean(biased_data$Con_RT)
...
data_grouped %>%
  ggplot(aes(x = Con_RT, fill = AttentionGroup, color = AttentionGroup))+
  geom_density(alpha = 0.3)+
...
data_grouped$IPS <- ifelse(data_grouped$BiasScore > 0, "Verbal", "Visual")

# Calculate means by attention and trial type
means <- data_grouped %>%
  group_by(AttentionGroup) %>%
...
means$AttentionGroup <- factor(means$AttentionGroup, levels = unique(means$AttentionGroup))

combined_long <- data_grouped %>%
  select(AttentionGroup, InCon_RT, Con_RT) %>%
  pivot_longer(cols = c(InCon_RT, Con_RT), values_to = "RT", names_to = "Condition")

# Creat error bars
se_sum <- combined_long %>%
  group_by(AttentionGroup, Condition) %>%
  summarise(
    sd = sd(RT),
    n = n(),
    mean = mean(RT)
...
# Create bar plot
ggplot(se_sum, aes(x = AttentionGroup, y = mean, fill = Condition)) +
...
combined_long_wid <- data_grouped %>%
  select(AttentionGroup, InCon_RT, Con_RT, Subject) %>%
  pivot_longer(cols = c(InCon_RT, Con_RT), values_to = "RT", names_to = "Condition")
```
Incongruent response times are larger overall, but the difference between incongruent and congruent trial response times for the biased group is a lot smaller than the neutral group. This may be because the neutral group is noticing both stimuli more.
...
# Create box plot
ggplot(combined_data, aes(x = AttentionGroup, y = Incongruency_Effect_Data, fill = AttentionGroup)) +
  geom_boxplot() +
  labs(
